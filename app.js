...
function userDetail(id){const p=state.users.find(x=>x.id===id); if(!p)return; const a=computedAttrs(p); openModal(`${modalHead(p.name||id)}<table class="detail-table"><tr><th>境界</th><td>${safe(p.realm)}</td></tr><tr><th>资质</th><td>${safe(p.aptitude)}</td></tr><tr><th>主流派</th><td>${safe(p.mainPath)} · ${safe(p.mainAttain)}</td></tr><tr><th>副流派</th><td>${safe(p.subPath)} · ${safe(p.subAttain)}</td></tr><tr><th>五维</th><td>生命${a.life} 攻击${a.attack} 防御${a.defense} 速度${a.speed} 精神${a.spirit}</td></tr><tr><th>元石</th><td>${n(p.stones)}</td></tr></table>`)}
